...
pular_argumento:
    ...
    li $t4, 39      # C�digo ASCII da aspa simples (')
    li $t5, 34      # C�digo ASCII da aspa dupla (")
    ...
    addi $t0, $t0, 1    # Avan�a na string
    j pular_argumento   # Continua verificando

finalizado_loop_sucesso:
	move $a0, $t0
	addi $a0, $a0, 1 # Pular o espa�o e as aspas
...
loop:
    ...
    sb $t3, 0($a2)  # Salva no buffer de sa�da
    addi $a0, $a0, 1 # Avan�a na string de entrada
    addi $a2, $a2, 1 # Avan�a no buffer de sa�da
...
strstr:
    move $t0, $a0       # Salva in�cio da string principal
loop_outer:
    lb $t1, 0($t0)      # Carrega um caractere da string principal
    beqz $t1, not_found # Se chegou ao fim, n�o encontrou a substring

    move $t2, $t0       # Ponteiro tempor�rio para a posi��o na string principal
...
loop_inner:
    ...
    beqz $t5, not_found # Se chegou ao fim da string principal, n�o encontrou
    bne $t4, $t5, next_outer # Se caracteres n�o coincidem, tenta pr�xima posi��o

    addi $t2, $t2, 1    # Avan�a na string principal
    addi $t3, $t3, 1    # Avan�a na substring
    j loop_inner        # Continua verificando

found_str:
    move $v0, $t0       # Retorna o endere�o da primeira ocorr�ncia
    jr $ra

next_outer:
    addi $t0, $t0, 1    # Avan�a o ponteiro na string principal
